#!/usr/bin/env python3
"""
THERMAL PULSE — Preset B: "Slow Burn"
Full-canvas flow field. No geometric boundary.
Particles fill every corner — an all-over composition.
146 years of flow layered like ocean currents or geological strata seen from orbit.
"""
...
class PerlinNoise:
    ...
    def noise3(self, x, y, z):
        X=np.floor(x).astype(int)&255; Y=np.floor(y).astype(int)&255; Z=np.floor(z).astype(int)&255
        x=x-np.floor(x); y=y-np.floor(y); z=z-np.floor(z)
        u=self._fade(x); v=self._fade(y); w=self._fade(z)
        p=self.p
        A=p[X]+Y; AA=p[A]+Z; AB=p[A+1]+Z; B=p[X+1]+Y; BA=p[B]+Z; BB=p[B+1]+Z
        return self._lerp(w,
            self._lerp(v,self._lerp(u,self._grad(p[AA],x,y,z),self._grad(p[BA],x-1,y,z)),
                         self._lerp(u,self._grad(p[AB],x,y-1,z),self._grad(p[BB],x-1,y-1,z))),
            self._lerp(v,self._lerp(u,self._grad(p[AA+1],x,y,z-1),self._grad(p[BA+1],x-1,y,z-1)),
                         self._lerp(u,self._grad(p[AB+1],x,y-1,z-1),self._grad(p[BB+1],x-1,y-1,z-1))))

# ─── Color ────────────────────────────────────────────────────────────
COLOR_STOPS = [
    (-0.50,(15,15,60)),(-0.25,(25,75,140)),(-0.05,(40,120,200)),
    (0.15,(70,170,220)),(0.35,(200,180,50)),(0.55,(240,130,30)),
    (0.75,(230,50,30)),(1.00,(255,60,20)),(1.30,(255,200,50)),
]
def anomaly_to_color(a):
    if a<=COLOR_STOPS[0][0]: return COLOR_STOPS[0][1]
    if a>=COLOR_STOPS[-1][0]: return COLOR_STOPS[-1][1]
    for i in range(len(COLOR_STOPS)-1):
        t0,c0=COLOR_STOPS[i]; t1,c1=COLOR_STOPS[i+1]
        if t0<=a<=t1:
            f=(a-t0)/(t1-t0); f=f*f*(3-2*f)
...
# ─── Render ───────────────────────────────────────────────────────────
def render():
    print(f"Rendering PRESET B: Slow Burn at {SIZE}x{SIZE}...")
    pnoise = PerlinNoise(SEED)
    pnoise2 = PerlinNoise(SEED + 77)  # second noise layer for flow variation
    img = Image.new('RGBA', (SIZE, SIZE), (5, 5, 26, 255))
    total = len(GISS)

    for yi, (year, anomaly) in enumerate(GISS):
        norm_a = (anomaly - MIN_A) / (MAX_A - MIN_A)
        time_frac = yi / (total - 1)

        col = anomaly_to_color(anomaly)
        num_p = int(PARTICLES_PER_YEAR * (0.5 + norm_a * 0.6))

        base_alpha = int(25 + time_frac * 65 + norm_a * 40)
        base_alpha = min(255, base_alpha)
        weight = max(1, int(1.0 + time_frac * 2.2 + norm_a * 1.5))

        steps = max(50, int(TRAIL_STEPS * (1.1 - norm_a * 0.3)))
        turb = 0.2 + norm_a * TURBULENCE
        ...
        for p in range(num_p):
            # Spawn ANYWHERE on the full canvas
            px = pseudo_random(yi, p, 0) * SIZE
            py = pseudo_random(yi, p, 1) * SIZE

            ns = NOISE_SCALE
            points = [(px, py)]
            for s in range(steps):
                # Two-layer noise for complex flow
                n1 = pnoise.noise3(px*ns + year_offset, py*ns, yi*0.02)
                n2 = pnoise2.noise3(px*ns*1.7 + 300, py*ns*1.7 + 300, yi*0.02 + 50)

                # Flow angle from noise — creates swirling currents
                flow_angle = (n1 + n2 * 0.5) * math.pi * 2 * (0.8 + turb * 0.3)

                # Speed varies with noise — creates density variation
                speed_mod = 0.6 + abs(n2) * 1.5
                spd = (1.8 + norm_a * 0.8) * speed_mod

                # Gentle drift bias — shifts with anomaly
                # Cool years: drift downward (settling). Hot years: drift upward (rising heat)
                drift_y = (norm_a - 0.3) * BREATHING_AMP * 0.3

                px += math.cos(flow_angle) * spd
                py += math.sin(flow_angle) * spd + drift_y

                # Wrap around edges for seamless fill
                px = px % SIZE
                py = py % SIZE

                points.append((px, py))

            # Draw trail — skip segments that wrap around (teleport)
            for s in range(len(points) - 1):
                x0, y0 = points[s]
                x1, y1 = points[s+1]
                # Skip wrap-around segments
                if abs(x1 - x0) > SIZE * 0.5 or abs(y1 - y0) > SIZE * 0.5:
                    continue
                fade = 1.0 - (s / len(points)) * 0.4
                alpha = max(1, min(255, int(base_alpha * fade)))
                draw.line([(x0,y0),(x1,y1)], fill=col+(alpha,), width=weight)

        img = Image.alpha_composite(img, layer)
        if (yi+1) % 10 == 0 or yi == total-1:
            print(f"  {year} ({int((yi+1)/total*100)}%)")

    # Post-processing — subtle, let the density speak
    print("  Post-processing...")
    glow = img.copy().filter(ImageFilter.GaussianBlur(radius=20))
    img_arr = np.array(img, dtype=np.float32)/255.0
    glow_arr = np.array(glow, dtype=np.float32)/255.0
    blended = 1.0 - (1.0 - img_arr) * (1.0 - glow_arr * 0.2)
    img = Image.fromarray((np.clip(blended, 0, 1)*255).astype(np.uint8), 'RGBA')
    ...
    vig = np.clip((dist - 0.85) * 2.5, 0, 1) ** 2
    bg = np.array([5, 5, 26], dtype=np.float32)
    for c in range(3):
        final_arr[:,:,c] = final_arr[:,:,c] * (1-vig*0.5) + bg[c] * vig * 0.5
    final = Image.fromarray(final_arr.astype(np.uint8), 'RGB')

    out = os.path.join(os.path.dirname(__file__), 'thermal-pulse-slow-burn.png')
