#!/usr/bin/env python3
"""
THERMAL PULSE — Preset A: "Last Gasp"
Vertical strata composition. Bottom=1880, Top=2025.
Particles flow horizontally, turbulence increases upward.
...
# ─── Render ───────────────────────────────────────────────────────────
def render():
    print(f"Rendering PRESET A: Last Gasp at {SIZE}x{SIZE}...")
    ...
    for yi, (year, anomaly) in enumerate(GISS):
        ...
        num_p = int(PARTICLES_PER_YEAR * (0.6 + norm_a * 0.8))

        base_alpha = int(30 + time_frac * 80 + norm_a * 50)
        base_alpha = min(255, base_alpha)
        weight = max(1, int(1.2 + time_frac * 2.5 + norm_a * 1.8))

        steps = max(20, int(TRAIL_STEPS * (1.2 - norm_a * 0.5)))
        turb = 0.15 + norm_a * TURBULENCE
        year_offset = yi * 137.508

        # Y band for this year — bottom of canvas = 1880, top = 2025
        band_y = SIZE * (1.0 - time_frac)  # flip: old=bottom, new=top
        band_height = SIZE / total * 2.5  # overlap between bands

        layer = Image.new('RGBA', (SIZE, SIZE), (0, 0, 0, 0))
        draw = ImageDraw.Draw(layer)

        for p in range(num_p):
            # Spawn across full width, within this year's band
            px = pseudo_random(yi, p, 0) * SIZE
            py = band_y + (pseudo_random(yi, p, 1) - 0.5) * band_height

            ns = NOISE_SCALE
            points = [(px, py)]
            for s in range(steps):
                n1 = pnoise.noise3(px*ns + year_offset, py*ns, yi*0.03)
                n2 = pnoise.noise3(px*ns + 500, py*ns + 500, yi*0.03 + 100)

                # Primary flow: horizontal (left to right)
                # Noise adds vertical displacement and speed variation
                flow_angle = n1 * math.pi * turb
                vertical_push = (n2 - 0.0) * turb * BREATHING_AMP
                # Hot years push particles upward (toward top/present)
                vertical_push -= norm_a * BREATHING_AMP * 0.5

                spd = 2.5 + norm_a * 1.5
                px += math.cos(flow_angle) * spd + spd * 0.8
                py += math.sin(flow_angle) * spd * 0.5 + vertical_push * 0.4

                points.append((px, py))

            for s in range(len(points) - 1):
                fade = 1.0 - (s / len(points)) * 0.5
                alpha = max(1, min(255, int(base_alpha * fade)))
                draw.line([points[s], points[s+1]], fill=col+(alpha,), width=weight)
    ...
    glow = img.copy().filter(ImageFilter.GaussianBlur(radius=25))
    img_arr = np.array(img, dtype=np.float32)/255.0
    glow_arr = np.array(glow, dtype=np.float32)/255.0
    blended = 1.0 - (1.0 - img_arr) * (1.0 - glow_arr * 0.25)
    img = Image.fromarray((np.clip(blended, 0, 1)*255).astype(np.uint8), 'RGBA')

    final = Image.new('RGB', (SIZE, SIZE), (5, 5, 26))
    final.paste(img, mask=img.split()[3])

    # Vertical gradient vignette (darken edges)
    final_arr = np.array(final, dtype=np.float32)
    yy, xx = np.mgrid[0:SIZE, 0:SIZE]
    edge_x = np.clip(np.minimum(xx, SIZE-1-xx) / (SIZE*0.15), 0, 1)
    edge_y = np.clip(np.minimum(yy, SIZE-1-yy) / (SIZE*0.08), 0, 1)
    vig = (edge_x * edge_y) ** 0.5
    bg = np.array([5, 5, 26], dtype=np.float32)
    for c in range(3):
        final_arr[:,:,c] = final_arr[:,:,c] * vig + bg[c] * (1-vig)
    final = Image.fromarray(final_arr.astype(np.uint8), 'RGB')

    out = os.path.join(os.path.dirname(__file__), 'thermal-pulse-last-gasp.png')
